  ...
  <body>
    ...
    <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
      <div class="container">
        ...
        <section @keypress.enter="addTicker">
          <div class="flex">
            <div class="max-w-xs">
              <label
                for="wallet"
...
export default {
  ...
  setup() {
    ...
    const addTicker = () => {
      ...
      setInterval(async () => {
        const res = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=b858c2a9bd5c32a72b75aee802ac525c81d006fd192c165bb194653bdb1e258b`
        );
        ...
        if (selectedTicker.value?.name === newTicker.name) {
    ...
    const deleteTicker = (t) =>
      (tickers.value = tickers.value.filter((w) => t !== w));
    const normalizeGraph = computed(() => {
      const maxValue = Math.max(...graph.value);
      const minValue = Math.min(...graph.value);

      return graph.value.map((usd) => {
        return maxValue !== minValue
          ? 5 + ((usd - minValue) * 95) / (maxValue - minValue)
          : "50";
      });
    });
